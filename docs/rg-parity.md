...
| `--search-zip` | `-z` | ⚠️ | Search in compressed files |
...
| `--encoding` | `-E` | ⚠️ | Specify the text encoding of files to search |
...
| `--pcre2` | `-P` | ⚠️ | Enable PCRE2 matching |
...
| `--json` |  | ⚠️ | Show search results in a JSON Lines format |
...
This file is generated — don't hand-edit the tables. The extraction
reads rg's `defs.rs` (`impl Flag` blocks: long/short/category/doc) and
gg's `cmd/gg/flags.go` (the implemented flag table vs the curated
`notImplementedFlags` list), asserts the two gg lists don't overlap,
and asserts gg has no flag rg lacks. Update the pins in "What is being
compared" when the rg checkout or gg release moves.
